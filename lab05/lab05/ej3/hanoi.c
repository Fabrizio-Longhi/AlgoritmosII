...
hanoi_t hanoi_init(unsigned int disk_count) {
    ...
    hanoi->target = stack_empty();          //add
    hanoi->source=stack_empty();            //add
...
hanoi_t hanoi_destroy(hanoi_t hanoi) {
    assert(hanoi != NULL);
    hanoi->source = stack_destroy(hanoi->source);       //change
    hanoi->aux = stack_destroy(hanoi->aux);             //change
    hanoi->target = stack_destroy(hanoi->target);       //change
